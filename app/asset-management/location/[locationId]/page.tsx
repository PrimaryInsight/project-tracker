...
export default async function LocationDetailPage({
  ...
  const locationId = decodeURIComponent(
    routeParameters.locationId
  ).trim();

  if (!user) {
    redirect(
      `/login?next=/asset-management/location/${encodeURIComponent(
        locationId
      )}`
    );
  }
  ...
  const locationResponse = await hardware
    .from("locations")
    .select(
      "location_id, client_id, location_name, location_type, region, latitude, longitude, status, verification_status, notes, created_at, updated_at"
    )
    .eq("location_id", locationId)
    .maybeSingle();

  const locationError =
    locationResponse.error?.message ?? null;
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          locationError ??
            `No location was found with ID ${locationId}.`
        ),
        h(
          "div",
          {
  ...
  const [
    ...
    hardware
      .from("installations")
      .select(
        "installation_id, client_id, location_id, installation_date, installation_type, crop, ftp_id, logger_id, logger_type, status, closed_at"
      )
      .eq("location_id", locationId)
    ...
    hardware
      ...
      .select(
        "installation_asset_id, installation_id, stock_item_id, product_code, product_description, product_serial, lifecycle_status, asset_classification, warehouse, client_id, client_name, location_id, location_name, current_position"
      )
      .eq("location_id", locationId)
      .order("stock_item_id"),
  ]);
  ...
  if (assetsResponse.error) {
    loadErrors.push(
      `Current assets: ${assetsResponse.error.message}`
    );
  }
    ...
    h(
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              `${installations.length} installations, ${activeInstallations.length} active, ${assets.length} currently assigned assets`
            )
          ),
          h(
            "span",
        ...
        mapUrl
          ? h(
              "a",
              {
                ...
                rel: "noreferrer",
                className:
                  "mt-3 inline-flex rounded-md border border-blue-300 bg-blue-50 px-3 py-1.5 text-sm font-semibold text-blue-800 hover:bg-blue-100",
              },
      ...
      h(
        ...
        installations.length === 0
          ? h(
              ...
              "No installations are recorded for this location."
            )
          : h(
              "div",
              {
      ...
      h(
        ...
        assets.length === 0
          ? h(
              ...
              "No current asset assignments were returned for this location."
            )
          : h(
              "div",
              {
      ...
      h(
        ...
        h(
          ...
          "This page retrieves location, client, installation and current asset information only. It does not create, edit or change any Hardware Database records."
        )
      )
    )
  );
